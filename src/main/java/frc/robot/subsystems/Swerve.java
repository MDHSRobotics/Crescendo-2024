...
public class Swerve extends SwerveDrivetrain implements Subsystem {
  ...
  private final SwerveRequest.SysIdSwerveTranslation TranslationCharacterization =
      new SwerveRequest.SysIdSwerveTranslation(); // Driving forward
  // private final SwerveRequest.SysIdSwerveRotation RotationCharacterization = new
  // SwerveRequest.SysIdSwerveRotation(); // Rotating robot (for HeadingController)
  // private final SwerveRequest.SysIdSwerveSteerGains SteerCharacterization = new
  // SwerveRequest.SysIdSwerveSteerGains(); // Rotating wheels only
  ...
  private final StructPublisher<Pose2d> camPosePublisher =
      table.getStructTopic("camPose", Pose2d.struct).publish();
  private final StructPublisher<Rotation2d> targetYawPublisher =
      table.getStructTopic("Target Direction", Rotation2d.struct).publish();
  ...
  public Rotation2d getTargetDirection(HeadingTargets target) {
    ...
    // If the alliance is red, the driveFacingAngle request will incorrectly try to rotate the
    // target direction, so rotate it back
  ...
  /**
   * Old method used to calculate the rotational rate based on the tx of the speaker. Only works if
   * a tag is in sight.
   *
  ...
  public double calculateTagRotationalRate() {
    ...
    double rotationalRate =
        MathUtil.clamp(
            output, -SwerveSpeedConstants.MaxAngularRate, SwerveSpeedConstants.MaxAngularRate);
   ...
   *
   * <p>Inspired by {@link com.ctre.phoenix6.mechanisms.swerve.SwerveModule#getPosition(boolean)
   * CTRE's getPosition method}
   *
  ...
  public double[] getWheelPositions() {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      // Back out the drive rotations based on angle rotations due to coupling between azimuth and
      // steer
  ...
  public void logData() {
    ...
    LimelightHelpers.SetRobotOrientation(
        "limelight-front", yawDegrees, yawRateDegrees, 0.0, 0.0, 0.0, 0.0);
    ...
    if (limelightMeasurement != null) {
      ...
      DriverStation.reportWarning(
          "Could not add limelight measurement to pose estimation, make sure limelight is properly connected and configured",
          false);
    }
  }
}
