...
public class RobotContainer {
  ...
  private final SwerveRequest.FieldCentric drive =
      new SwerveRequest.FieldCentric()
          ...
          .withRotationalDeadband(SwerveSpeedConstants.MaxAngularRate * 0.06);
  ...
  private final SwerveRequest.PointWheelsAt pointWheelsForward =
      new SwerveRequest.PointWheelsAt().withModuleDirection(Rotation2d.fromDegrees(0));
  ...
  public RobotContainer() {
    ...
    // If you use kI in any PID controller, or kD in a Phoenix PID Controller, you must call reset()
    // before you start aiming to prevent a large spike in output.
    ...
    // Enable continuous angle input, so that the robot doesn't spin around to go from 180 to -180
    // degrees.
    ...
    tagIsInSight
        ...
        .whileTrue(s_Led.blinkCommand(255, 0, 0, 300).withName("Blink LEDs Red"));

    // When the shooter is ready, turn the LEDs green
    shooterIsReady.whileTrue(s_Led.setColorCommand(0, 255, 0).withName("Green LEDs"));
  ...
  private void configureDriverButtonBindings() {
    ...
    */
    ...
    driverController
        ...
        .onTrue(
            s_Swerve
                .runOnce(() -> s_Swerve.seedFieldRelative())
                    ...
                    s_Swerve.applyRequest(
                        () ->
                            driveFacingAngle
                                ...
                                .withVelocityY(getVelocityY() * 0.5)
                                .withTargetDirection(
                                    s_Swerve.getTargetDirection(HeadingTargets.SOURCE))))
                    ...
                    s_Swerve.applyRequest(
                        () ->
                            driveFacingAngle
                                ...
                                .withVelocityY(getVelocityY() * 0.5)
                                .withTargetDirection(
                                    s_Swerve.getTargetDirection(HeadingTargets.STAGE_LEFT))))
                .until(this::driverAttempedToRotate)
                    ...
                    s_Swerve.applyRequest(
                        () ->
                            driveFacingAngle
                                ...
                                .withVelocityY(getVelocityY() * 0.5)
                                .withTargetDirection(
                                    s_Swerve.getTargetDirection(HeadingTargets.STAGE_RIGHT))))
                .until(this::driverAttempedToRotate)
                    ...
                    s_Swerve.applyRequest(
                        () ->
                            driveFacingAngle
                                ...
                                .withVelocityY(getVelocityY() * 0.5)
                                .withTargetDirection(
                                    s_Swerve.getTargetDirection(HeadingTargets.STAGE_MIDDLE))))
                .until(this::driverAttempedToRotate)
    ...
    driverController
        .L1()
        .and(climbLimitSwitchesPressed.negate())
        .whileTrue(s_Climb.runClimbCommand(-1, -1).withName("Lower Both Climbs"));

    driverController.R1().whileTrue(s_Climb.runClimbCommand(1, 1).withName("Raise Both Climbs"));

    driverController
        .povLeft()
        .whileTrue(s_Climb.runClimbCommand(1, 0).withName("Raise the Left Climb"));

    driverController
        .povRight()
        .whileTrue(s_Climb.runClimbCommand(0, 1).withName("Raise the Right Climb"));
  ...
  private void configureOperatorButtonBindings() {
    ...
    */
    ...
    operatorController
        ...
        .toggleOnTrue(
            Commands.race(
                    s_Intake.intakeNoteCommand(IntakePositions.MID), s_Shooter.intakeNoteCommand())
                        ...
                        s_Swerve.applyRequest(
                            () ->
                                driveFacingAngle
                                    ...
                                    .withVelocityY(getVelocityY())
                                    .withTargetDirection(
                                        s_Swerve.getTargetDirection(HeadingTargets.SPEAKER)))),
                        ...
                        s_Swerve.applyRequest(
                            () ->
                                driveFacingAngle
                                    ...
                                    .withVelocityY(getVelocityY())
                                    .withTargetDirection(
                                        s_Swerve.getTargetDirection(HeadingTargets.AMP_AREA)))),
    ...
    operatorController
        .povLeft()
        .toggleOnTrue(s_Shooter.manualAimCommand(operatorController::getRightY));
    ...
    operatorController
        .back()
        .toggleOnTrue(
            Commands.race(s_Intake.ejectNoteCommand(), s_Shooter.ejectNoteCommand())
                .withName("Eject Note"));
  }
  ...
  private void registerPathplannerCommands() {
    ...
    // ALSO, Triggers can still activate in autonomous, so do not add any Trigger-activated commands
    // that require subsystems used in autonomous.
    ...
    NamedCommands.registerCommand(
        ...
        Commands.sequence(
            // Angle the shooter
            s_Shooter.aimShooterWithPoseCommand(s_Swerve::getPose).withTimeout(0.75),
            // Run the shooter
            s_Shooter.shootNoteCommand(),
            // Disable the shooter
    ...
    // This command expects the intake to already be at the bottom position, so no time is wasted
    // with waiting.
    NamedCommands.registerCommand(
        "Run intake only",
        Commands.parallel(
