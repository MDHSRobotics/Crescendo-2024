...
public class Telemetry {
    ...
    private final StructArrayPublisher<SwerveModuleState> moduleTargetsPublisher = table.getStructArrayTopic(
                    "Module Targets", SwerveModuleState.struct)
            .publish();
    ...
    private final DoublePublisher speedPublisher =
            table.getDoubleTopic("Robot Speed").publish();
    ...
    public void telemeterize(SwerveDriveState state) {
        ...
        double speed =
                Math.sqrt(Math.pow(state.speeds.vxMetersPerSecond, 2) + Math.pow(state.speeds.vyMetersPerSecond, 2));
